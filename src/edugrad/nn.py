...
class Linear(Module):
    ...
    def __init__(
        ...
        if self.has_bias:
            self.bias = Tensor(
                uniform_initializer((output_size,), scale=scale), name="b"
            )

    def forward(self, inputs: Tensor) -> Tensor:
        mul_node = ops.matmul(inputs, self.weight)
        if self.has_bias:
            # copy_rows expands bias from (output_size,) to (batch, output_size)
